...
export default function EditGroceryModal({ item, onClose }: EditGroceryModalProps) {
  const { updateGrocery } = useGroceryStore();
  ...
  useEffect(() => {
    if (item) {
      ...
      setBrand(item.brand || '');
      setCategory(item.category || 'Dairy & Eggs');
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    if (imageFile) {
      ...
      // If image preview was cleared, remove the image
      formData.append('image', '');
    }

    // Call store update grocery
    const success = await updateGrocery(item._id, formData);
    setIsSubmitting(false);

    if (success) {
      onClose();
    } else {
      setErrors({ api: 'Failed to update grocery. Please check details and try again.' });
    }
  };
    ...
    <AnimatePresence>
      <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-50 flex items-end md:items-center justify-center p-0 md:p-4">
        ...
        <motion.div 
          ...
          exit={{ opacity: 0 }}
          onClick={() => {
            if (!isSubmitting && !isWebcamActive) onClose();
          }}
        />
          ...
          <div className="px-5 py-3.5 border-b border-zinc-800 flex justify-between items-center select-none bg-zinc-900/50">
            <div>
              ...
              <p className="text-[9px] text-zinc-500 font-semibold mt-0.5 uppercase tracking-widest">
                Modify existing storage details
              </p>
            </div>
            <button 
              onClick={() => {
