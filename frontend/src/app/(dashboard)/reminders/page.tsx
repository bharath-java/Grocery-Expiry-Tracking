...
export default function RemindersPage() {
  ...
  // Render settings switches reusable block
  const renderSettingsPreferencesBlock = (theme: 'light' | 'dark') => {
    const textColor = theme === 'dark' ? 'text-zinc-300' : 'text-slate-700';
    const subColor = theme === 'dark' ? 'text-zinc-500' : 'text-slate-400';
    const bgCard = theme === 'dark' ? 'bg-zinc-900/40 border-zinc-900' : 'bg-slate-50 border-slate-100';

    return (
      <div className="space-y-4">
        <div>
          <h4 className="text-[10px] font-black uppercase tracking-widest text-[#2E7D32]">
            Alert Intervals
          </h4>
          <p className="text-[9px] font-semibold text-slate-450 dark:text-zinc-500 uppercase mt-0.5">
            Decide when notifications should trigger
          </p>
        </div>

        <div className="space-y-2.5">
          {/* 7 Days */}
          <div className={`flex items-center justify-between p-3.5 border rounded-2xl ${bgCard}`}>
            ...
            <button
              ...
              className={`w-9 h-5.5 rounded-full p-0.5 transition-all duration-200 relative cursor-pointer ${
                intervals.sevenDays ? 'bg-[#2E7D32]' : 'bg-slate-200 dark:bg-zinc-800'
              }`}
            >
              <div 
          ...
          <div className={`flex items-center justify-between p-3.5 border rounded-2xl ${bgCard}`}>
            ...
            <button
              ...
              className={`w-9 h-5.5 rounded-full p-0.5 transition-all duration-200 relative cursor-pointer ${
                intervals.threeDays ? 'bg-[#2E7D32]' : 'bg-slate-200 dark:bg-zinc-800'
              }`}
            >
              <div 
                className={`w-4.5 h-4.5 bg-white rounded-full shadow-sm transform transition-transform duration-200 ${
          ...
          <div className={`flex items-center justify-between p-3.5 border rounded-2xl ${bgCard}`}>
            ...
            <button
              ...
              className={`w-9 h-5.5 rounded-full p-0.5 transition-all duration-200 relative cursor-pointer ${
                intervals.oneDay ? 'bg-[#2E7D32]' : 'bg-slate-200 dark:bg-zinc-800'
              }`}
            >
              <div 
                className={`w-4.5 h-4.5 bg-white rounded-full shadow-sm transform transition-transform duration-200 ${
          ...
          <div className={`flex items-center justify-between p-3.5 border rounded-2xl ${bgCard}`}>
            ...
            <button
              ...
              className={`w-9 h-5.5 rounded-full p-0.5 transition-all duration-200 relative cursor-pointer ${
                intervals.sameDay ? 'bg-[#2E7D32]' : 'bg-slate-200 dark:bg-zinc-800'
              }`}
            >
              <div 
                className={`w-4.5 h-4.5 bg-white rounded-full shadow-sm transform transition-transform duration-200 ${
  ...
  return (
    <div className="select-none h-full flex flex-col justify-between">
      ...
      <div className="hidden lg:flex flex-col gap-6 flex-grow w-full">
        ...
        <div>
          ...
          <p className="text-xs font-semibold text-slate-400 dark:text-zinc-500 mt-0.5">
            Configure system notification thresholds and view kitchen logs history
          </p>
        </div>

        {/* 2-Column Split: Alerts Feed & Settings Preferences Card */}
        <div className="grid grid-cols-12 gap-6 flex-grow items-start min-h-0">
          
          {/* Left Feed Pane */}
          <div className="col-span-8 bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800/80 rounded-2xl shadow-sm flex flex-col h-full max-h-[580px]">
            {/* Tabs inside Left feed header */}
            <div className="flex justify-between items-center p-4 border-b border-slate-100 dark:border-zinc-800/60">
              <div className="flex bg-slate-50 dark:bg-zinc-850 p-1 rounded-xl border border-slate-150 dark:border-zinc-800/50">
                <button
                  onClick={() => setActiveSubTab('upcoming')}
                  className={`px-4 py-2 text-[10px] font-black tracking-widest uppercase rounded-lg transition-all cursor-pointer ${
                    activeSubTab === 'upcoming'
                      ? 'bg-white dark:bg-zinc-800 text-[#2E7D32] shadow-sm'
                      : 'text-slate-400 dark:text-zinc-500 hover:text-slate-600'
                  }`}
                >
                  Upcoming Alarms ({upcomingItems.length})
                </button>
                <button
                ...
                {activeSubTab === 'upcoming' ? (
                  ...
                  >
                    {upcomingItems.length === 0 ? (
                      <div className="flex flex-col items-center justify-center py-16 text-center border-2 border-dashed border-slate-100 dark:border-zinc-800/80 rounded-2xl">
                        <Bell className="w-8 h-8 text-slate-300 dark:text-zinc-650 mb-3 animate-pulse" />
                        <h4 className="text-xs font-black text-slate-800 dark:text-zinc-300 uppercase tracking-wider">
                          No Upcoming Alerts
                        </h4>
                        <p className="text-[10px] text-slate-400 dark:text-zinc-500 font-bold uppercase tracking-widest mt-1">
                          Everything in your kitchen is fresh.
                        </p>
                      </div>
                    ) : (
                        ...
                        {upcomingItems.map(item => {
                          ...
                          return (
                            <div
                              ...
                              className="flex items-center gap-3.5 bg-slate-50 dark:bg-zinc-850/50 border border-slate-100 dark:border-zinc-800/60 p-4 rounded-2xl hover:bg-slate-100/50 dark:hover:bg-zinc-800/50 transition-colors cursor-pointer"
                            >
                              <div className="w-10 h-10 rounded-xl bg-green-500/10 border border-green-500/20 flex items-center justify-center text-brand-green">
                                <Bell className="w-5 h-5 fill-current" />
                              </div>
                              <div className="min-w-0 flex-1">
                                <h4 className="text-sm font-extrabold text-slate-800 dark:text-zinc-200 truncate leading-none">
                                  {item.itemName}
                                </h4>
                                <span className="text-[10px] font-bold text-slate-400 dark:text-zinc-500 block mt-2">
                                  {formattedAlarm}
                                </span>
                              </div>
                            </div>
                  ...
                  >
                    {loadingHistory ? (
                      ...
                      <div className="flex flex-col items-center justify-center py-16 text-center border-2 border-dashed border-slate-100 dark:border-zinc-800/80 rounded-2xl">
                        <CheckCircle2 className="w-8 h-8 text-slate-300 dark:text-zinc-650 mb-3" />
                        <h4 className="text-xs font-black text-slate-800 dark:text-zinc-300 uppercase tracking-wider">
                          Logs Feed is Empty
                        </h4>
                        <p className="text-[10px] text-slate-400 dark:text-zinc-500 font-bold uppercase tracking-widest mt-1">
                          No historical notification alerts.
                        </p>
                      </div>
                    ) : (
                      <div className="space-y-3">
                        {notifications.map(n => (
                          <div
                            key={n._id}
                            onClick={() => {
                              if (n.groceryId) setSelectedProduct(n.groceryId);
                            }}
                            className="bg-slate-50 dark:bg-zinc-850/50 border border-slate-100 dark:border-zinc-800/60 rounded-2xl p-4 flex justify-between items-start gap-4 hover:bg-slate-100/50 dark:hover:bg-zinc-800/50 transition-colors cursor-pointer"
                          >
                            <div className="flex items-start gap-3.5 min-w-0">
                              <div className="mt-0.5 flex-shrink-0">
                                {n.type === 'expiry' ? (
                                  <div className="w-8 h-8 bg-amber-50 dark:bg-amber-950/20 rounded-xl flex items-center justify-center text-brand-orange border border-amber-100 dark:border-amber-900/50">
                                    <AlertTriangle className="w-4 h-4" />
                                  </div>
                                ) : (
                                  <div className="w-8 h-8 bg-emerald-50 dark:bg-green-950/20 rounded-xl flex items-center justify-center text-primary dark:text-green-400 border border-green-100 dark:border-green-900/50">
                                    <CheckCircle2 className="w-4 h-4" />
                                  </div>
                                )}
                              </div>
                              <div className="min-w-0 flex-1">
                                <h4 className="text-sm font-black text-slate-800 dark:text-zinc-200 leading-none">
                                  {n.title}
                                </h4>
                                <p className="text-xs text-slate-500 dark:text-zinc-400 font-semibold leading-relaxed mt-1.5">
                                  {n.message}
                                </p>
                                <span className="text-[9px] text-slate-400 dark:text-zinc-500 block mt-2 font-bold uppercase tracking-wider">
                            ...
                            <button
                              ...
                              className="p-2 text-slate-350 dark:text-zinc-650 hover:text-red-500 dark:hover:text-red-400 hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-xl cursor-pointer transition-all flex-shrink-0"
                            >
          ...
          {/* Right Preferences Pane (Rendered inline!) */}
          <div className="col-span-4 bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800/80 rounded-2xl shadow-sm p-6 space-y-4">
            <div className="flex items-center gap-2 pb-3 border-b border-slate-100 dark:border-zinc-800/60">
              ...
              <h3 className="text-xs font-black uppercase tracking-wider text-slate-800 dark:text-zinc-200 leading-none">
                Preferences Card
              </h3>
            </div>
            
            {/* Inline preferences switches */}
            {renderSettingsPreferencesBlock('light')}
          </div>
      ...
      <div className="lg:hidden flex flex-col h-full overflow-hidden justify-between space-y-3 pb-1">
              ...
              >
                {upcomingItems.length === 0 ? (
                  <div className="bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800/80 border-dashed rounded-2xl p-7 text-center">
                    ...
                    <h4 className="text-[10px] font-black text-slate-855 dark:text-zinc-300 uppercase tracking-widest leading-none">
                      No Upcoming Alarms
                    </h4>
        ...
        {/* Reminders Preferences Modal Sheet overlay (⚙️ Settings control panel) */}
        <AnimatePresence>
          {showSettings && (
            <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-50 flex items-end justify-center p-0">
              ...
              <motion.div
                className="relative bg-zinc-950 w-full rounded-t-custom shadow-2xl flex flex-col overflow-hidden z-10 border-t border-zinc-800"
                initial={{ y: '100%' }}
                animate={{ y: 0 }}
                exit={{ y: '100%' }}
              ...
              >
                ...
                <div className="p-5 space-y-4 max-h-[60vh] overflow-y-auto no-scrollbar">
                  {renderSettingsPreferencesBlock('dark')}
                </div>

                <div className="px-5 py-4 border-t border-zinc-800 bg-zinc-900/30">
                  <button
                    onClick={() => setShowSettings(false)}
                    className="w-full bg-zinc-850 hover:bg-zinc-800 text-zinc-100 py-3 rounded-xl text-[9px] font-black tracking-widest uppercase transition-colors cursor-pointer border border-zinc-700/60"
                  >
                    SAVE & CLOSE
                  </button>
                </div>
